...
<div class="ticket-form">
    ...
    $start = $model->isNewRecord ? null : $form->field($model, 'start')->widget(DateTimePicker::classname(), [
        'options' => ['placeholder' => \Yii::t('tickets', 'Enter start time ...')],
        'pluginOptions' => [
    ...
    $end = $model->isNewRecord ? null : $form->field($model, 'end')->widget(DateTimePicker::classname(), [
        'options' => ['placeholder' => \Yii::t('tickets', 'Enter end time ...')],
        'pluginOptions' => [
            'format' => 'yyyy-mm-dd hh:ii:ss',
            'todayHighlight' => true,
            'todayBtn' => true,
    ...
    $backup_interval = $form->field($model, 'backup_interval', [
        'template' => '{label}<div class="input-group">{input}<span class="input-group-addon" id="basic-addon2">' . \Yii::t('tickets', 'seconds') . '</span></div>{hint}{error}'
    ])->textInput(['type' => 'number']);
    $time_limit = $form->field($model, 'time_limit', [
        'template' => '{label}<div class="input-group">{input}<span class="input-group-addon" id="basic-addon2">' . \Yii::t('tickets', 'minutes') . '</span></div>{hint}{error}'
    ])->textInput(['type' => 'number']);
    ...
    <div class="form-group">
        <?= Html::submitButton($model->isNewRecord ? \Yii::t('tickets', 'Create') : \Yii::t('tickets', 'Apply'), ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ?>
    </div>
    <?php } else { ?>
