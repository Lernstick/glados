...
if ($model->type == 'dir') {
    ...
    echo Html::a(
        '<span class="glyphicon glyphicon-list-alt"></span> ' . \Yii::t('tickets', 'View all versions'),
        Url::to([
            false,
            'id' => $ticket->id,
            'path' => $model->path,
    ...
    echo " ";
    echo Html::a(
        '<span class="glyphicon glyphicon-tasks"></span> ' . \Yii::t('tickets', 'Restore this state of the file'),
        Url::to([
            'ticket/restore',
            'id' => $ticket->id,
            'file' => $model->path,
...
if ($model->type == 'dir') {
    if ($model->state == 'missing') {
        echo '<span class="text-muted">' . yii::$app->formatter->format($model->version, 'backupVersion') . ', <abbr title="' . \Yii::t('tickets', 'This directory does not exist in that version. Restoring it in this state will REMOVE the directory from the target machine.') . '">' . $model->state . '</abbr></span>';
    }
} else {
    if ($model->state != 'missing') {
        echo '<span class="text-muted">' . ($model->version == $model->newestBackupVersion ? '(' . \Yii::t('tickets', 'current') . ') ' : null) . yii::$app->formatter->format($model->version, 'backupVersion') . ', ' . yii::$app->formatter->format($model->mode, 'text') . ', ' . yii::$app->formatter->format($model->size, 'shortSize') . '</span>';
    } else {
        echo '<span class="text-muted">' . yii::$app->formatter->format($model->version, 'backupVersion');
        if (!($model->wasWhiteout == true && $model->isOldestVersion)) {
            echo ', <abbr title="' . \Yii::t('tickets', 'This file does not exist in that version. Restoring it in this state will REMOVE the file from the target machine.') . '">' . $model->state . '</abbr>';
        }
        echo '</span>';
    }
}
