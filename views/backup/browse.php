  ...
  <div class="panel-heading">
    <div class="row">
      <div class="col-sm-8 backup-browse-item">
        ...
        <div class="">
        <?= Html::a(
            '<span class="glyphicon glyphicon-' . ($options['showDotFiles'] ? 'check' : 'unchecked') . '"></span> ' . \Yii::t('ticket', 'Show hidden files'),
            Url::to([
                false,
                'id' => $ticket->id,
                'path' => $fs->path,
...
<?php
        ...
        echo '<span>' . \Yii::t('ticket', 'Path') . '</span>: ' . Html::a(
        ...
        if ($date != "all") {
          ...
          echo Html::a(
              '<span class="glyphicon glyphicon-tasks"></span> ' . \Yii::t('ticket', 'Restore this state of the directory'),
              Url::to([
                  'ticket/restore',
                  'id' => $ticket->id,
                  'file' => $fs->path,
...
?>
      ...
      <div class="col-sm-4">
        ...
        <div class="input-group input-group-sm col-sm-12 pull-right special-dropdown">
          <span class="input-group-addon" id="version-addon" style="min-width:70px;"><?= \Yii::t('ticket', 'Version') ?></span>
          <span class="input-group-btn" style="width:100%;">
            <button style="width:100%;" class="btn btn-default toggle-dropdown" data-toggle="dropdown" aria-expanded="false" aria-haspopup="true" aria-describedby="version-addon"> <span class="glyphicon glyphicon-cog"></span> <?= $date == 'all' ? \Yii::t('ticket', 'All versions overlapping') : (yii::$app->formatter->format($fs->version, 'datetime') . ($fs->version == $fs->newestBackupVersion ? ' (' . \Yii::t('ticket', 'current') . ')' : null)); ?></button>
            ...
            <?= ListView::widget([
                ...
                'emptyText' => \Yii::t('ticket', 'No versions.'),
                'emptyTextOptions' => [
                    'tag' => 'li',
    ...
    <?= ListView::widget([
        ...
        'emptyText' => \Yii::t('ticket', 'No files or directories.'),
        'emptyTextOptions' => [
            'tag' => 'div',
