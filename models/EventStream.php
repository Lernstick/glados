...
class EventStream extends EventItem
{
    ...
    /*
     * The [[listenEvents]] array is modified whether there is an event defined to listen on multiple
     * events/files.
     * @return void
     */
    private function setupWatches(){
        
        foreach($this->listenEvents as $event){
            if(basename($event) == "*"){
                $files = scandir('/tmp/user/' . dirname($event));
                foreach($files as $file){
                    if(is_file('/tmp/user/' . dirname($event) . '/' . $file)){
                        $this->listenEvents[] = dirname($event) . '/' . $file;
                    }
                }
                $this->listenEvents[] = dirname($event) . '/' . 'ALL';
            }
        }
        $this->listenEvents = array_unique($this->listenEvents);
    }
    ...
    public function onEvent(){
        while($this->calcTimeout() > 0){
            ...
            echo '0'; ob_flush(); flush();
            file_put_contents('/tmp/test2', '0' . PHP_EOL, FILE_APPEND);
