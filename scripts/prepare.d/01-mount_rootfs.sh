...
{
  >&2 echo "mounting root filesystem"
  ...
  if [ -d "${filesystem_squashfs}" ]; then
    mount -v --bind "${filesystem_squashfs}" "${initrd}"/base
  else
    mount -v "${filesystem_squashfs}" "${initrd}"/base
  fi

  if [ -e ${initrd}/squashfs/exam.squashfs ]; then
    mount -v ${initrd}/squashfs/exam.squashfs ${initrd}/exam
  ...
  if [ -e ${initrd}/squashfs/exam.zip ]; then
    mount -v -t tmpfs tmpfs ${initrd}/tmpfs
  ...
  # | base (ro)     |
  # +---------------+
  mkdir -pv ${initrd}/work
  if [ "${type}" = "aufs" ]; then
    # in there are whiteouts for aufs (\.wh\.*) and no whiteouts for overlayfs (character devices with 0/0) it is an aufs filesystem
    mount -v -t aufs -o br=${initrd}/backup=rw:${initrd}/tmpfs=ro:${initrd}/exam=ro:${initrd}/base=ro none "${initrd}/${newroot}"
    cat <<EOF >"${mountFile}"
mount -v -t aufs -o br=/backup=rw:/tmpfs=ro:/exam=ro:/base=ro none "/${newroot}"
EOF
  else
    # in all other cases the filesystem in treated as overlay
    mount -v -t overlay overlay -o lowerdir=${initrd}/tmpfs:${initrd}/exam:${initrd}/base,upperdir=${initrd}/backup,workdir=${initrd}/work ${initrd}/${newroot}

    # remount /run with bigger size (50% of physical RAM)
    mount -v -n -o remount,size=50% /run

    cat <<EOF >"${mountFile}"
mount -v -t overlay overlay -o lowerdir=/tmpfs:/exam:/base,upperdir=/backup,workdir=/work /${newroot}
EOF
  fi
}
